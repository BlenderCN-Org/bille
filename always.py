...
def main():
    ...
    if gl.phase == "début du jeu":
        debut_jeu(all_obj)
           
    if gl.phase == "jeu avant retard":
        jeu_avant_retard(all_obj)

    if gl.phase == "retard":
        retard(all_obj)

    if gl.phase == "jeu après retard":
        jeu_apres_retard(all_obj)

    if gl.phase == "fin de jeu":
        fin_jeu(all_obj)

    if gl.phase == "début de fin":
        debut_fin(all_obj)

    if gl.phase == "fin":
        fin(all_obj)

    if gl.phase == "fin de fin":
        fin_de_fin(all_obj)
        sleep(3)
